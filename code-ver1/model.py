...
class PolicyHead(nn.Module):
    def __init__(self, planes, board_size):  # 128, 15
        super(PolicyHead, self).__init__()
        self.policy_head = nn.Conv2d(planes, 2, kernel_size=1, bias=False)
        self.policy_bn = nn.BatchNorm2d(2)
        self.relu = nn.ReLU()
        self.policy_fc = nn.Linear(board_size**2 * 2, board_size**2)  #입력값 왜 이럴까?
...
class PVNet(nn.Module):
    def __init__(self, n_block, inplanes, planes, board_size):  
        ...
        self.policy_head = PolicyHead(planes, board_size)  # 128, 15
        self.value_head = ValueHead(planes, board_size)   # 128, 15
